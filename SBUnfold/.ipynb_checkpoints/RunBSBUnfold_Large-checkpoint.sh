...
srun python train_physics.py --corrupt 'BSBUnfold_OmniFold_train_large_KL1fix_iter10M_6x256' --n-gpu-per-node 1  --beta-max 0.1 --sample-name 'OmniFold_train_large.h5' --num-itr 1000000 --kl 1.0 --hidden-dim 256 --nresnet 6

srun python sample.py --n-gpu-per-node 1  --sample-name 'OmniFold_train_large.h5'  --ckpt 'BSBUnfold_OmniFold_train_large_KL1fix_iter10M_6x256' --batch-size 10000 --save-target '/global/cfs/projectdirs/m3246/sdiefenbacher/Schroedinger_Bridge/SBUnfold_results/BSBUnfold' --n-ensemble 1 --n-ensemble-start 0 --n-eval 5309543 --hidden-dim 256 --nresnet 6 --bnn-map
