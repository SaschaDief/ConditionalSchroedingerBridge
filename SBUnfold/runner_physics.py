...
import util
from network import DenseNet, TransNet, TransNetV2, TransNetV3
...
def build_optimizer_sched(opt, net, log):
    ...
    if opt.lr_cos_anneal:
        sched = lr_scheduler.CosineAnnealingLR(optimizer, opt.num_itr)
        log.info(f"[Opt] Built lr CosineAnnealingLR scheduler !") 
    elif opt.lr_gamma < 1.0 and not opt.lr_cos_anneal:
...
class Runner_physics(object):
    def __init__(self, opt, log, save_opt=True):
        ...
        if opt.transV3:
            self.net = TransNetV3(
                ...
                x_dim=22, 
                time_embed_dim=4,
                dim_embedding=opt.dim_embedding,
            ...
            self.net = DenseNet(
                ...
                nresnet=opt.nresnet,
                nlayers_in_resnet=opt.layerresnet,
            )
    ...
    def train(self, opt, train_dataset, val_dataset):
        ...
        for it in range(opt.num_itr):
            ...
            for _ in range(n_inner_loop):
                ...
                ce_loss = F.mse_loss(pred, label)
                if opt.bayes:
                    kl = self.get_kl_loss()
                    loss = ce_loss + (opt.kl * (kl / len(train_dataset)))
                else:
                    loss = ce_loss
                loss.backward()
    ...
    def bayes_reset_random(self):       
        for name, module in self.net.named_modules():
            if 'VBLinear' in str(type(module)):
                module.reset_random()
                
    def bayes_set_map(self):       
        for name, module in self.net.named_modules():
            if 'VBLinear' in str(type(module)):
                module.map=True
